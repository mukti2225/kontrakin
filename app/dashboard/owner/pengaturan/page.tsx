...
import { getOrCreateKodeUndangan } from "./action";

export default async function SettingsPage() {
  const kode = await getOrCreateKodeUndangan();
  return (
    <div className="flex-1 space-y-4 p-4 md:p-8 pt-6">
      ...
      <Tabs defaultValue="profil" className="space-y-4 mt-6">
        ...
        {/* Konten Tab Preferensi (Opsional) */}
        <TabsContent value="preferensi">
          <Card>
            <CardHeader>
              <CardTitle>Kode Undangan Penghuni</CardTitle>
              <CardDescription>Bagikan kode ini ke penghuni agar mereka bisa mendaftar melalui dashboard tenant.</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="text-3xl font-bold tracking-widest">{kode}</div>
            </CardContent>
          </Card>
        </TabsContent>
      </Tabs>
